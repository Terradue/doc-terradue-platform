    ...
    public class OAuthEverestService : ServiceStack.ServiceInterface.Service
    {
        ...
        public object Get (OauthEverestCallBackRequest request)
        {
            ...
            try {
                ...
                user.Store ();
                user.ChangeLdapPassword (client.EVEREST_TOKEN_ACCESS, null, true);//update token on LDAP as it was regenerated by the AccessToken function

                //Create the session also on SSO
                var clientSSO = new Connect2IdClient (context.GetConfigValue ("sso-configUrl"));
                ...
                clientSSO.SetCookie (Connect2IdClient.COOKIE_SUB_SID, directAuthResponse.sub_sid);
                clientSSO.SetCookie (Connect2IdClient.COOKIE_TOKEN_ACCESS, directAuthResponse.access_token, directAuthResponse.expires_in);
                clientSSO.SetCookie (Connect2IdClient.COOKIE_TOKEN_REFRESH, directAuthResponse.refresh_token);

                OauthAuthzPutSessionRequest oauthrequest2 = new OauthAuthzPutSessionRequest {
                    sub = user.Username
                };
                var oauthputsession = clientSSO.AuthzSession (clientSSO.SID, oauthrequest2, false);
